...
def parse_args():
    ...
    parser.add_argument(
        "--stop-hub-on-exit", action="store_true",
        help="explicitly stop the hub program when the host exits",
    )
...
async def run(args):
    ...
    try:
        ...
        print("Hold the robot upright and release at the readiness beep. "
              "Axis 1 drives; axis 2 steers. Begin with very small commands. "
              "The hub CENTER button stops balancing. Ctrl-C disconnects the "
              "host but leaves hub balancing active.")
...
def main():
    try:
        ...
        print("\nHost disconnected; hub balancing remains active.")
        return 0
